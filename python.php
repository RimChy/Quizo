...
if(isset($_POST['submit'])){
	...
	for($i=1;$i<16;$i++){
				...
				$stmt=$pdo->prepare('Select * from question_python where id=:qid');
				$stmt->execute(array(
				':qid'=>$i
				));
...
<html>
	...
	<body>
	  ...
	  <div class="collapse navbar-collapse" id="navbarNav">
		...
		<div class="container">
		<h1>Good Luck <?=htmlentities($_SESSION['name'])?> for your Python quiz</h1>
		<h3>15 Questions</h3>
		<div class="card border-primary mb-3 mx-auto text-left" style="width:50%;">;
		 ...
		 <?php
			...
			$stmt1=$pdo->prepare('Select * from ans_python where question_id=:qid');
				$stmt1->execute(array(
				':qid'=>$i
				));
				...
				while($rows1=$stmt1->fetch(PDO::FETCH_ASSOC)){
					...
					echo ('<input type="radio" name="quizcheck['.htmlentities($rows1['question_id']).']" value="'.htmlentities($rows1['id']).'"/>');
			...
			<footer class="bg-dark text-white py-2 mt-3">Reference:Some questions are from <a href="https://www.w3schools.com/">w3schools.com</a><footer>
